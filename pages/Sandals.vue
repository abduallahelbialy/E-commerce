  ...
  methods: {
    async fetchProducts() {
      try {
        ...
        this.Kids = data.products; // storage the data
        this.shuffleSandals();
    ...
    shuffleSandals() {
      for (let i = this.Kids.length - 1; i > 0; i--) {
        ...
        [this.Kids[i], this.Kids[j]] = [this.Kids[j], this.Kids[i]];
      }
    },
    ...
    loadMore() {
      if (this.visibleItems + 4 <= this.Kids.length) {
        this.visibleItems += 4;
        this.updateDisplayedSandals();
      } else {
this.visibleItems = this.Kids.length; // Show all remaining items
        this.updateDisplayedSandals();
      }
    },
    searchProducts() {
      if (this.searchQuery.trim() === "") {
this.visibleItems = 4; // Reset the number of visible items
        this.updateDisplayedSandals();
      } else {
        const query = this.searchQuery.toLowerCase();
        const filteredItems = this.Kids.filter((item) => {
          return (
        ...
        this.displayedSandals = filteredItems.slice(0, this.visibleItems);
      }
    },
  },
};
